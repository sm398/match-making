...
module.exports =
    {
         ...
         * @returns {Promise<String>} A Promise that returns userId
         */
        ...
        getGame: function(userId)
        {
            return new Promise(function(fulfill, reject)
            {
                db.filter({remainingPlayers: 0, players: userId}, collectionName).then(function(games)
                {
                    if(games.length !== 0)
                    {
                        fulfill(games[0]);
         ...
         * @param {String} userId - The user that will be removed
        ...
        removeUserFromGame: function(userId, gameId)
        {
            return new Promise(function(fulfill, reject)
            {
                ...
                {
                    uc.removeGameFromUser(userId).then(function(res)
                    {
                        fulfill(userId);
                    });
                });
